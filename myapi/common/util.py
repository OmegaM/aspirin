...
def check_resource_exists(model):
    def _check_resource_exists(req_func):
        def __check_resource_exists(self, id):
            if not model.query.get(id):
                abort(404, message=str(model.__name__) + " id = {} doesn't exist".format(id))
            else:
                return req_func(self, id)
        return __check_resource_exists
    return _check_resource_exists
